...
if __name__ == "__main__":
    # code to read image
    imageb = cv.imread('img_gray.jpg',0)
    #cv.imshow("Original",image)
    smoothed_imageb = smooth(imageb)
    #cv.imshow("GaussinSmooth(5*5)",smoothed_image)
    gradientsb, directionb = get_gradient_and_direction(smoothed_imageb)
    # print(gradients)
    # print(direction)
    nmsb = NMS(gradientsb, directionb)
    output_imageb = double_threshold(nmsb, 30,60)
    cv.imwrite("img_canny-1.jpg", output_imageb)
    
    #Canny算法实现参考了文章：
    #https://yueyue200830.github.io/2020/04/04/Python实现Canny算子边缘检测/
